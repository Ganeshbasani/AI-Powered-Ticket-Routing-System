...
RUN python -c "from pathlib import Path; import shutil; src=Path('frontend/src'); dist=Path('frontend/dist'); shutil.rmtree(dist, ignore_errors=True); shutil.copytree(src, dist)"
