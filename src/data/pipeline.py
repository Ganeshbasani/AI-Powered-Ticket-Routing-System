...
def profile_markdown(frame: pd.DataFrame, report: ValidationReport) -> str:
    ...
    return "\n".join(["# Dataset Profile", "", f"- Rows: {len(frame)}", f"- Schema version: {SCHEMA_VERSION}", f"- Validation status: {'valid' if report.valid else 'invalid'}", f"- Missing values: {report.missing_values}", f"- Exact duplicates: {report.duplicate_rows}", f"- Priority distribution: {distribution('priority')}", f"- Issue-type distribution: {distribution('issue_type')}", f"- SLA outcome distribution: {distribution('actual_sla_breach')}", f"- Text availability: {text_available}", f"- Timestamp coverage: {int(frame['created_timestamp'].notna().sum()) if 'created_timestamp' in frame else 0}", "", "## Leakage Warnings", *warning_lines, "", "## Limitation", "The bundled development dataset is synthetic; replace it with representative historical data before making production performance claims."])
